...
path = os.getcwd()
path_v = '../04_visuals/youtube_clips/'
vidcap = FileVideoStream(path_v + args["video_file"]).start()
count = 0
framecount = 0
...
face_cascade = cv2.CascadeClassifier("../01_materials/haarcascade/haarcascade_frontalface_default.xml")
...
predictor = dlib.shape_predictor("../01_materials/predictor/shape_predictor_68_face_landmarks.dat")
(lStart, lEnd) = face_utils.FACIAL_LANDMARKS_IDXS["left_eye"]
(rStart, rEnd) = face_utils.FACIAL_LANDMARKS_IDXS["right_eye"]

if args["save"]:
    out = cv2.VideoWriter(args["video_file"].split('.')[0] + "_output.avi", cv2.VideoWriter_fourcc('M','J','P','G'), 10, (450,300))
...
while vidcap.more():
    ...
    for coords in face_coord:
        ...
        cv2.putText(frame, str(prediction), (X_1, Y_1 + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (225, 255, 255), 2)
...
if args["savedata"]:
    ...
    df.to_csv('../01_materials/exported_materials/video_csv/'+ args["video_file"].split('.')[0]+'_export.csv')
