...
ap.add_argument("-v", "--video_file", help="video file in current directory")
ap.add_argument("--frame_step", type=int, default = 10, help="framecount which video frames are predicted")
ap.set_defaults(video_file = False)
...
path = os.getcwd()
#path = 'C:/Users/JDN/Desktop/Github/FacialExpressionRecognition'
learn = load_learner(path, 'export.pkl')

face_cascade = cv2.CascadeClassifier("haarcascade_frontalface_default.xml")

if args["video_file"]:
    vs = VideoStream(src= args["video_file"],framerate=1).start()
else:
...
while True:
    ...
    for coords in face_coord:
        ...
        cv2.putText(frame, str(prediction), (X_1, Y_1), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (225, 255, 255), 2)
